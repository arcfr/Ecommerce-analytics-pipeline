SQL_QUERIES = {
    ...
    "customer_tier_monthly_count": """
        ...
        SELECT order_month, tier, COUNT(customer_id) AS customer_count
        FROM customer_segmentation
        GROUP BY order_month, tier
        ORDER BY order_month ASC;
    """,
    "lifetime_value_quartiles": """
    ...
    "yoy_revenue_comparison": """
        ...
        SELECT cur.r_year AS year, cur.r_month AS month, ROUND(cur.revenue, 2) AS revenue,
               ROUND(prev.revenue, 2) AS prev_year_revenue,
               ROUND(((cur.revenue - COALESCE(prev.revenue, 0)) / NULLIF(prev.revenue, 0)) * 100.0, 2) AS yoy_growth_percent
        FROM monthly_rev cur
        LEFT JOIN monthly_rev prev ON cur.r_year = prev.r_year + 1 AND cur.r_month = prev.r_month;
    """,
    "category_shift_analysis": """
        ...
        SELECT f.customer_id, f.first_category, l.most_recent_category,
    ...
    "revenue_concentration": """
        ...
        total_aggregate AS (
            SELECT SUM(revenue) AS global_revenue FROM customer_spend
        ),
        running_spend AS (
        ...
        SELECT r.customer_id, ROUND(r.revenue, 2) AS revenue, ROUND(r.cumulative_revenue, 2) AS cumulative_revenue,
               ROUND((r.cumulative_revenue / t.global_revenue) * 100.0, 2) AS cumulative_percent
        FROM running_spend r
        CROSS JOIN total_aggregate t;
    """,
    "cohort_retention_matrix": """
        ...
        SELECT rc.cohort_month, cs.total_users, rc.month_number, rc.active_users,
